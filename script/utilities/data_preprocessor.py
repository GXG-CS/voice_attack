...
def process_folder(folder_path, label, executor):
    features_list = []
    # List and sort files based on the integer `{i}` in `{i}_{xxx}.pcap`
...
def write_features_to_csv(features_list, csv_file_path):
    with open(csv_file_path, 'w', newline='') as file:
        if features_list:
            fieldnames = ['label'] + list(features_list[0][0].result().keys())
...
def main():
    ...
    with ProcessPoolExecutor() as executor:
        features_futures = []
        subdirs = [d for d in os.listdir(args.pcap_folder) if os.path.isdir(os.path.join(args.pcap_folder, d))]
        subdirs_sorted = sorted(subdirs, key=lambda x: int(x.split('_')[-1]))
        for subdir in subdirs_sorted:
            subdir_path = os.path.join(args.pcap_folder, subdir)
            label = subdir.split('_')[-1]
            features = process_folder(subdir_path, label, executor)
            features_futures.extend(features)
        features_list = [(future, label) for future, label in features_futures]
        write_features_to_csv(features_list, args.csv_file_path)

if __name__ == "__main__":
    main()


# python data_preprocessor4.py --pcap_folder "../../data_processed/vc_200/alexa/outgoing" --csv_file_path "../../data_processed/vc_200/alexa/combined_features_outgoing.csv"