...
def parse_pcap(file_path):
    # Parse the pcap file to extract packet sizes and times
...
def calculate_features(sizes, times):
    # Calculate various features from packet sizes and times
    if sizes.size == 0 or times.size == 0:
        return None
    
    # Feature calculations including statistical metrics and signal processing measures
...
def extract_features_from_pcap(file_path):
    # Wrapper function for parsing pcap files and calculating features
    sizes, times, _ = parse_pcap(file_path)
    return calculate_features(sizes, times)

def process_file(file_path):
    # Process each pcap file, extract features and handle errors
    print(f"Processing file: {file_path}")
    features, error = extract_features_from_pcap(file_path), None
    return features, error

def process_folder(folder_path, label, executor):
    # Process each folder corresponding to a label, extracting features from pcap files
    print(f"Processing folder: {folder_path}, Label: {label}")
    features_list = []
    file_paths = sorted(glob(os.path.join(folder_path, f'{label}_*', '*.pcap')))
...
def get_labels(folder_path):
    # Extract labels from the folder names, considering only the number before '_'
    print(f"Getting labels from folder: {folder_path}")
    subfolders = [name for name in os.listdir(folder_path) if os.path.isdir(os.path.join(folder_path, name)) and '_' in name]
    labels = sorted(set(subfolder.split('_')[0] for subfolder in subfolders), key=int)
...
def main(incoming_folder, outgoing_folder, csv_file_path):
    # Main function to process incoming and outgoing folders, and write features to CSV
...
# python data_preprocessor2.py --incoming_folder ../../data_processed/categories/categories_incoming --outgoing_folder ../../data_processed/categories/categories_outgoing --csv_file_path ../../data_processed/categories/categories.csv